...
- `TIGER_ACCOUNT_METHOD` (optional Tiger gateway method for account query, default `get_assets` with fallback candidates)
- `TIGER_POSITIONS_METHOD` (optional Tiger gateway method for positions query, default `get_positions` with fallback candidates)
