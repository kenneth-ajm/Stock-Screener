...
async function runSectorSingle(opts: {
  ...
  try {
    ...
    return {
      ...
      rows_new: Math.max(0, after - before),
      duration_ms: Date.now() - startedAt,
      before_count: before,
      after_count: after,
...
async function runChunkedBatch(opts: {
  ...
  try {
    ...
    return {
      ...
      universe: opts.universe,
      scan_date_used: opts.scanDate,
      rows_written: Number((out as any)?.upserted ?? Math.max(0, after - before)),
      rows_new: Math.max(0, after - before),
      duration_ms: Date.now() - startedAt,
      before_count: before,
      after_count: after,
...
async function runByMode(req: Request, body: any) {
  ...
  if (mode === "single") {
    ...
    const response = {
      ...
      phase: "single",
      bars_mode: "cached_db_only",
      scan_context_key: `${strategy}:${universe}:${scanDate}`,
  ...
  if (mode === "batch") {
    ...
    const batchSizeRaw = Number(body?.batch_size ?? 25) || 25;
    const batchSize = Math.min(40, Math.max(10, batchSizeRaw));
