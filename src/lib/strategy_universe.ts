...
export const UNIVERSE_LABELS: Record<string, string> = {
  [CORE_UNIVERSE_SLUG]: "Established Leaders",
  [LEGACY_MOMENTUM_UNIVERSE_SLUG]: "Broad Liquid US",
  [MIDCAP_UNIVERSE_SLUG]: "Mid-Cap Opportunities",
  [GROWTH_UNIVERSE_SLUG]: "Legacy Growth 1500",
};
...
export function defaultUniverseForStrategy(strategyVersion: string) {
  ...
  if (strategy === SECTOR_MOMENTUM_STRATEGY_VERSION) return LEGACY_MOMENTUM_UNIVERSE_SLUG;
...
export function allowedUniversesForStrategy(strategyVersion: string) {
  ...
  if (strategy === SECTOR_MOMENTUM_STRATEGY_VERSION) return [LEGACY_MOMENTUM_UNIVERSE_SLUG, MIDCAP_UNIVERSE_SLUG];
